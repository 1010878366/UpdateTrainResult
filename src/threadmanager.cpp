...
void ThreadManager::setDBTaskParams(const QString &reelTable, ConfigManager *cfgMgr, DatabaseManager *dbMgr, bool isAuto)
{
...
void ThreadManager::WriteToDB()
{
    ...
    if(m_isAuto)
        emit logMessage(tr("数据表【%1】完成 自动 更新，共 %2 条记录").arg(m_strReelTable).arg(nCount));
    else
        emit logMessage(tr("数据表【%1】完成 手动 更新，共 %2 条记录").arg(m_strReelTable).arg(nCount));
